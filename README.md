...
Claude Code 任务完成 BLE 提示灯。硬件：ESP32-C6（板载 GPIO8 WS2812 + GPIO3 9g 舵机挥手）。主机：macOS + `uv` + `pyobjc`（CoreBluetooth）。

## 动机

Claude Code 执行任务的时间经常长到让人切走去做别的事，回来才发现已经完成。系统通知容易漏，而桌面上一颗会呼吸发光的 LED 是"离开屏幕也能看到"的物理提醒；再加一只会挥手的小舵机，连余光都不用扫——动作直接进入外周视觉。
...
└─────────────────┘                        └────────┬─────────────┘
                                    ...
                                    LocalName="bclock:CC:NNNN..."
                                        ...
                                        │  ┌──────────────────┐  │
                                        │  │ Notifier         │  │
                                        │  │ ├ AnimationRunner│──┼──▶ GPIO8 WS2812 呼吸
                                        │  │ └ ServoRunner    │──┼──▶ GPIO3 9g 舵机挥手
                                        │  └──────────────────┘  │
                                        └────────────────────────┘
```
...
- **`firmware/*.py`**：设备端 MicroPython，`aioble.scan` 100% duty cycle 被动扫描，匹配 LocalName 前缀 → 解析命令/nonce → `Notifier` 把命令分发给 `AnimationRunner`（WS2812 动画）和 `ServoRunner`（舵机挥手），两条 asyncio task 独立取消、互不阻塞
...
| state     | 字节 | LED                     | 舵机              | 触发 hook                   | 清除方式                              |
|-----------|------|-------------------------|-------------------|----------------------------|---------------------------------------|
| done      | 0x01 | 🟢 绿色呼吸×3 (~3s)     | 👋 挥手 ×3 (~2s)  | `Stop` / `SubagentStop`    | 自动播放完                            |
| error     | 0x02 | 🔴 红色三闪 (~1.2s)     | 👋 挥手 ×3 (~2s)  | 手动 / 未来扩展             | 自动                                  |
| plan      | 0x03 | 🟦 冰蓝呼吸（持续）     | 不动（保持中位）  | `PreToolUse(ExitPlanMode)` | `PostToolUse(ExitPlanMode)` → off     |
| attention | 0x04 | 🟦 深蓝呼吸（持续）     | 👋 挥手 ×3 (~2s)  | `Notification`             | 下一个 `Stop` → done → 灭             |
| off       | 0x00 | ⚫ 熄灭                 | 回中 90°          | —                          | —                                     |
...
firmware/         ESP32-C6 MicroPython 固件（config / led / servo / servo_anim / ble_server / main）
servo_demo/      舵机独立调试样例（不参与烧录，只用于上手验证舵机接线）
```
...
- 9g 舵机（SG90 等）一只，外接 5V 电源

**接线：**

| 信号       | ESP32-C6 引脚 | 备注                                    |
|------------|---------------|-----------------------------------------|
| WS2812 DIN | GPIO8（板载） | 一般无需外接                            |
| 舵机 PWM   | GPIO3         | 50Hz / 500-2500µs，固件初始化即拉到 90° |
| 舵机 VCC   | 外部 5V       | **不要**接 ESP32 的 3V3，电流不够会 brownout 重启 |
| 舵机 GND   | 共地          | 舵机 GND 必须与 ESP32 GND 接通          |
...
**舵机不动 / 抖动 / ESP32 不停重启：**
- 不动：先用 `servo_demo/main.py` 单独跑（`mpremote run servo_demo/main.py`）确认接线和供电没问题，再回到固件
- 抖动或嗡鸣：通常是 5V 供电压降，换更稳的电源或加 ≥470µF 电解电容到舵机 VCC-GND
- ESP32 一上电就重启：几乎一定是把舵机 VCC 接到了 3V3，改外部 5V 即可
